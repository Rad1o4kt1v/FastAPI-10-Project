...
# Конфигурация JWT
SECRET_KEY = "your-secure-random-secret-key"  # Replace with a secure key (e.g., `openssl rand -hex 32`)
ALGORITHM = "HS256"
...
class User(Base):
    ...
    role = Column(String, default="user")  # Поле role с дефолтным значением "user"
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
    to_encode = data.copy()
    if expires_delta:
        expire = datetime.now(timezone.UTC) + expires_delta  # Fixed deprecation warning
    else:
...
# Функция проверки токена и получения текущего пользователя
...
# Эндпоинт миграции существующих паролей
...
# Защищенный эндпоинт для администраторов: список всех пользователей
